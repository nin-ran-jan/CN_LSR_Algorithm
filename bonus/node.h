...
// class TrafficMsg {
//     public:
//         unordered_map<pair<Node*, Node*>, int> traffic_map;
//         Node* from_where;
//         int seq_num;
// };
...
class Node {
    ...
    protected:
        ...
        unordered_map<Node *, int> traffic_map;
    ...
    public:
        ...
        void sendTraffic(Node *nd, int traffic){
            if (nd == this){
                return;
            }
            for(int i = 0; i < mytbl.tbl.size(); i++){
                for(int j = 0; j < nd->interfaces.size(); j++){
                    if(mytbl.tbl[i].dstip == nd->interfaces[j].first.getip()){
                        string cur_ip = mytbl.tbl[i].ip_interface;
                        string next_hop = mytbl.tbl[i].nexthop;
                        Node * nxt;
                        for(auto it : interfaces){
                            if (it.first.getConnectedIp() == next_hop){
                                nxt = it.second;
                            }
                        }
                        traffic_map[nxt] += traffic;
                        nxt->sendTraffic(nd, traffic);
                        return;
                    }
                }
            }
        ...
        void modifyInterfaces(){
            for(int i = 0; i < interfaces.size(); i++){
                interfaces[i].first.setCost(0);
            }

            // for(auto it : interfaces){
            //     cout << it.first.getCost();
            // }

            for (auto it : traffic_map){
                for (int i = 0; i < interfaces.size(); i++){
                    if(it.first == interfaces[i].second){
                        interfaces[i].first.setCost(it.second);
                    }
                }
            }
        }
